...
import { LOGIN, PATIENT_DASHBOARD_REPORT,ROOT } from '../../../Components/navigation/Constant'
        ...
        <div className='flex min-h-screen w-screen overflow-hidden overflow-x-hidden'>
            ...
            <div className='w-1/2 container mx-auto flex flex-col items-center mt-[200px]'>
                ...
                <div className='flex flex-col gap-[10px] card w-[80%]md:w-[40%] shadow-[0px_0px_8px_rgba(0,0,0,0.1)] bg-[#ffffff] py-[30px] px-[20px] rounded-[8px]  mx-auto'>
                        ...
                        <Input label="Phone number" name="email" type="number" placeholder="Enter your Phone number" className=" rounded-[12px]" value={email} handleChange={(e: any) => setEmail(e.target.value)}/>
                         {
                            otpSent ? <>
                                ...
                                <button className="bg-blue-600 text-[#fff] text-[14px] px-[45px] mx-auto font-semibold rounded-[12px] py-[8px] mt-[15px]" onClick={handleLogin}>
                                Verify OTP
                                </button>
                            </> : null
                        }
                    ...
                    <p className='text-[#000] text-center text-[#000]/[0.7] mt-[15px] font-medium'>Want to Sign In with UID? <span className='text-right text-blue-600 hover:underline cursor-pointer' onClick={() => {
                        navigate(ROOT)
                    }}>Click here</span>.</p>
                </div>
